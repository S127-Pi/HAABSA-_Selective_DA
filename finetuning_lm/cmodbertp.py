...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, seed=12345):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if ex_index < 2:
            ...
            logger.info("label_len: %s" % str(label_len))
...
def prepare_data(features):
    ...
    all_masked_lm_labels = torch.tensor([f.masked_lm_labels for f in features],
                                        dtype=torch.long)
    all_label_lengths = torch.tensor([f.label_length for f in features],
                                        dtype=torch.long)
...
def main():
    ...
    parser.add_argument("--output_dir", default="finetuned_model/BERTP", type=str,
                        help="The output dir for augmented dataset")
    parser.add_argument("--task_name",default="sem",type=str,
    ...
    print(args)
    train_cmodbertp_and_augment(args)
...
def augment_train_data(model, tokenizer, train_data, label_list, args):
    ...
    best_model_path = os.path.join(args.output_dir, "best_cmodbertp.pt")
    if os.path.exists(best_model_path):
        model.load_state_dict(torch.load(best_model_path))
    else:
        raise ValueError("Unable to find the saved model at {}".format(best_model_path))

    save_train_path = os.path.join(args.output_dir, "cmodbertp_aug.tsv")
    ...
    for step, batch in enumerate(train_dataloader):
        model.eval()
        batch = tuple(t.to(device) for t in batch)
        init_ids, _, input_mask, _, label_lengths = batch
        input_lens = [sum(mask).item() for mask in input_mask]
        masked_idx = np.squeeze(
            [np.random.randint(1 + label_lengths[i].cpu().numpy(), l,
                               max((l - (1 + label_lengths[i].cpu().numpy())) // args.sample_ratio, 1))
        ...
        lower_label_list = [x.lower() for x in label_list]        
        for ids, idx, preds, label_len in zip(init_ids, masked_idx, predictions, label_lengths):
            ...
            for pred in preds:
                ...
                for x in range(label_len):
                    label_cand += '{}'.format(new_str[1+x][2:] if new_str[1+x].startswith('##') else new_str[1+x])
                label = label_list[lower_label_list.index(label_cand.lower())]
                new_str = rev_wordpiece(new_str)
                tsv_writer.writerow([label, new_str])


def train_cmodbertp_and_augment(args):
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        if dev_loss < best_dev_loss:
            ...
            save_model_path = os.path.join(args.output_dir, 'best_cmodbertp.pt')
